...
/**
 * main - this program the information contained in the
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	sm_read = read(sm_open, header, sizeof(Elf64_Ehdr));
	if (sm_read == -1)
	{
		free(header);
		close_elf(sm_open);
 ...
 *
 * @e_ident: this is a pointer parameter to an array containing
...
void print_class(unsigned char *e_ident)
{
	...
	for (int i = 0; i < sizeof(e_ident); i++)
	{
		if (i == EI_CLASS)
		{
			switch (e_ident[i])
