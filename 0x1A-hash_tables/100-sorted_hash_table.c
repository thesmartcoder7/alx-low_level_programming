 ...
 *
 * Return:  NULL on error else a pointer to the new sorted hash table.
 */
shash_table_t *shash_table_create(unsigned long int size)
{
	shash_table_t *ht;
...
/**
 * shash_table_get - gets the value assigned to a
 *                   key in a sorted hash table.
 * @ht: A pointer to the sorted hash table.
 * @key: The key for which to get the value.
 *
 * Return: NULL if key is unmatched else the value to the key.
 */
char *shash_table_get(const shash_table_t *ht, const char *key)
{
	shash_node_t *node;
